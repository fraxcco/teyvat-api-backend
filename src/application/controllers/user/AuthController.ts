...
import { IApiKey, IUser, AuthRequest } from "../../../components/interfaces";
import { asyncHandler, CustomError } from "../../../infrastructure/middleware";
import { verifyAccessToken, generateToken } from "../../../shared/utils/jwtUtils";
...
export class AuthController {
    ...
    public refreshToken = asyncHandler(async (req: Request, res: Response) => {
        ...
        try {
            decoded = jwt.verify(refreshToken, environment.JWT_REFRESH_SECRET, { algorithms: ["HS256"] }) as jwt.JwtPayload;
        } catch {
            return res.status(HTTP_STATUS.UNAUTHORIZED).json({
                success: false,
    ...
    public createApiKey = asyncHandler(async (req: Request, res: Response) => {
    ...
    public listApiKeys = asyncHandler(async (req: Request, res: Response) => {
        const userId = (req as AuthRequest).user.id;
    ...
    public revokeApiKey = asyncHandler(async (req: Request, res: Response) => {
        const userId = (req as AuthRequest).user.id;
        ...
        const query: Partial<IApiKey> = { userId: userId };

        if(key) query.key = key;
        if(label) query.label = label;

        const result = await this.apiKeyService.deleteApiKeyByQuery(query as FilterQuery<IApiKey>);
